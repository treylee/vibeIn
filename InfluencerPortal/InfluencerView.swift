...
struct InfluencerView: View {
    ...
    var body: some View {
        NavigationStack {
            ZStack {
                // Vibe gradient background - matching BizzSearchView
                LinearGradient(
                ...
                VStack(spacing: 20) {
                    // Header matching BizzSearchView style
                    VStack(spacing: 16) {
                        ZStack {
                            Circle()
                                .fill(
                        ...
                        Text("Discover offers and events")
                    ...
                    // Search Bar matching BizzSearchView style
                    HStack {
                        Image(systemName: "sparkle.magnifyingglass")
            ...
            .navigationDestination(isPresented: Binding(
                ...
                set: { if !$0 { selectedOffer = nil } }
            )) {
                if let offer = selectedOffer {
                    InfluencerRestaurantDetailView(offer: offer)
                }
            }
        }
    }
    ...
    private func setupInfluencerProfile() {
        // The influencer service will auto-create a profile on init
        if let influencer = influencerService.currentInfluencer {
            print("👤 Influencer profile loaded: \(influencer.userName)")
        }
    }
}
